...
//输出为
#######第一种状态切换 -新建-》运行-》终止################################
没调用start的 thread1的状态NEW
thread1 当前状态 RUNNABLE
thread1 running
thread1的状态TERMINATED

########第二种状态切换 -> new -> runnable -> timed_waiting -> terminated ###########################
没调用start之前thread2线程的状态NEW
调用start之后thread2线程的状态RUNNABLE
主线程睡了200ms之后thread2的状态TIMED_WAITING
thread2 当前状态RUNNABLE
thread2 running
主线程睡了3000ms之后thread2的状态TERMINATED

############第三种状态切换  -> new -> runnable -> blocked -> running###########################
调用start之前thread3的状态NEW
调用start之后thread3的状态RUNNABLE
主线程睡了200ms之后thread3的状态BLOCKED
thread3 当前状态RUNNABLE
thread3 running
主线程睡了3s之后thread3的状态TERMINATED
```
